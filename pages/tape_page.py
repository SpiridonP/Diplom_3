...
class Tape(BasePage):
    ...
    def check_users_order(self):
        button_go_to_account = self.wait_and_find(locators.TapePageLocators.ACCOUNT)
        button_go_to_account.click()
        email = self.wait_and_find(locators.TapePageLocators.EMAIL_INPUT)
        email.send_keys(data.HELPERS.email_for_test)
        password = self.wait_and_find(locators.TapePageLocators.PASSWORD_INPUT)
        password.send_keys(data.HELPERS.password)
        button_log_in = self.wait_and_find(locators.TapePageLocators.LOG_IN_BUTTON)
        button_log_in.click()
        ...
        order = self.wait_and_find(locators.TapePageLocators.ORDER_BUTTON)
        order.click()
        exit = self.wait_and_find(locators.TapePageLocators.BUTTON_EXIT)
        exit.click()
        tape_button = self.wait_and_find(locators.TapePageLocators.ORDER_TAPE)
        tape_button.click()
    ...
    def increase_all_time(self):
        ...
        button_constructor = self.wait_and_find(locators.TapePageLocators.BUTTON_CONSTRUCTOR)
